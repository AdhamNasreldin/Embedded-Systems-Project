#include "io.h"
#include <stdint.h>
#include "tm4c123gh6pm.h" // Tiva C series header file
...
void init_GPIO(void) {
    ...
    GPIO_PORTF_DIR_R |= 0x08; // Set PF3 as output (bit 3)
    GPIO_PORTF_DEN_R |= 0x08; // Enable digital function for PF3 (bit 3)

    // Set PF4 (push button) as input
    GPIO_PORTF_DIR_R &= ~0x10; // Set PF4 as input (bit 4)
